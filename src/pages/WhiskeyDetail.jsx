...
import { isRateable as stageRateable } from '../lib/stages'
...
export default function WhiskeyDetail() {
  ...
  const isRateable = whiskey ? stageRateable(whiskey, currentStage) : false
  ...
  return (
    <div className="px-5 py-6 max-w-lg mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        {whiskey.image_url && (
          <div className="flex justify-center mb-6 candle-glow rounded-2xl py-5">
            <motion.img layoutId={`bottle-${whiskey.id}`} src={whiskey.image_url} alt={whiskey.name} className="h-56 object-contain" style={{ mixBlendMode: 'screen', filter: 'drop-shadow(0 12px 24px rgba(0,0,0,0.6))' }} />
          </div>
        )}

        <h1 className="font-heading text-3xl font-semibold text-foreground">{whiskey.name}</h1>
        {whiskey.is_centrepiece && (
          <span className="inline-block mt-2 text-[11px] uppercase tracking-widest text-primary font-semibold bg-primary/10 px-3 py-1 rounded-full">
            Centrepiece
          </span>
        )}
        ...
        <div className="mt-8 mb-4">
          {isRateable ? (
            <Button
              ...
              className="w-full h-12 font-heading text-base shadow-warm"
            >
              Rate This Whiskey
            </Button>
          ) : (
